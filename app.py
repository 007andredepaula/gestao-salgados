...
st.set_page_config(
    page_title="Sistema de Gestão Integrada - Fábrica de Salgados", layout="wide"
)
...
def inicializar_banco():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.execute(
            "INSERT INTO regionais (cidade, meta_mensal) VALUES ('Guarapari', 100000.0)"
        )
...
def verificar_acesso():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT aprovado FROM dispositivos_autorizados WHERE device_id = ? AND aprovado = 1",
        (dev_id,),
    )
...
if perfil in ["Funcionário", "Gerente"]:
    if not verificar_acesso():
        st.error(
            "🚨 Dispositivo não autorizado. Solicite o link de acesso único ao Administrador."
        )
        st.stop()
...
if perfil == "Administrador":
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Segurança e Dispositivos", "Metas Regionais", "Financeiro & Bônus"]
    )
    ...
    with tab1:
        ...
        if st.button("Gerar Novo Link"):
            ...
            cursor.execute(
                "INSERT INTO tokens_acesso (token, loja_id, status, expiracao) VALUES (?, ?, 'pendente', ?)",
            ...
            st.info(
                "Envie o link acima para o gerente. Ele expira em 24h e só funciona uma vez."
            )
...
st.sidebar.divider()
st.sidebar.caption(
    f"Status do Dispositivo: {'🟢 Autorizado' if verificar_acesso() else '🔴 Bloqueado'}"
)
